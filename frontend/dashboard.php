                    ...
                    <div id="settings-tab-company" class="bkm-settings-tab-content">
                        <div class="bkm-management-grid">
                            ...
                            <div class="bkm-management-list">
                                ...
                                <div class="bkm-company-info" id="company-info-display">
                                    <?php 
                                    $company_name = get_option('bkm_company_name', '');
                                    $company_logo = get_option('bkm_company_logo', '');
                                    $company_address = get_option('bkm_company_address', '');
                                    $company_phone = get_option('bkm_company_phone', '');
                                    $company_email = get_option('bkm_company_email', '');
                                    ?>
                                    
                                    <?php if ($company_name || $company_logo): ?>
                                        <div class="bkm-company-header">
                                            <?php if ($company_logo): ?>
                                                <div class="bkm-company-logo-display">
                                                    <img src="<?php echo esc_url($company_logo); ?>" alt="<?php echo esc_attr($company_name); ?>" />
                                                </div>
                                            <?php endif; ?>
                                            <?php if ($company_name): ?>
                                                <h5><?php echo esc_html($company_name); ?></h5>
                                            <?php endif; ?>
                                        </div>
                                        
                                        <div class="bkm-company-details">
                                            <?php if ($company_address): ?>
                                                <p><strong>📍 Adres:</strong> <?php echo esc_html($company_address); ?></p>
                                            <?php endif; ?>
                                            <?php if ($company_phone): ?>
                                                <p><strong>📞 Telefon:</strong> <?php echo esc_html($company_phone); ?></p>
                                            <?php endif; ?>
                                            <?php if ($company_email): ?>
                                                <p><strong>📧 E-posta:</strong> <?php echo esc_html($company_email); ?></p>
                                            <?php endif; ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="bkm-no-company-info">
                                            <p><em>Henüz firma bilgileri eklenmemiş.</em></p>
                                            <p>Lütfen firma bilgilerini doldurun.</p>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
...
function loadUsersList() {
    ...
    // Frontend.js'teki loadUsers fonksiyonunu çağır
    if (typeof window.loadUsers === 'function') {
        window.loadUsers();
    } else {
        console.error('❌ loadUsers fonksiyonu bulunamadı!');
        // Fallback olarak kendi implementasyonumuzu kullan
        loadUsersManual();
    }
}

// Manuel kullanıcı yükleme fonksiyonu (fallback)
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (response && response.success && response.data) {
                ...
                if (users.length === 0) {
                    html = '<div class="bkm-no-items">⚠️ Kullanıcı bulunamadı.<br><small>Editör veya Katılımcı rolüne sahip kullanıcı bulunmamaktadır.</small></div>';
                } else {
