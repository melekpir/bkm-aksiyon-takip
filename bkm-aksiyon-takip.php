...
public function ajax_add_user() {
    ...
    // Sadece editör ve katılımcı rollerine izin ver
    if (!in_array($role, array('editor', 'contributor'))) {
        wp_send_json_error('Geçersiz rol. Sadece Editör ve Katılımcı rolleri eklenebilir.');
    }
...
public function ajax_edit_user() {
    ...
    // Sadece editör ve katılımcı rollerine izin ver
    if (!in_array($role, array('editor', 'contributor'))) {
        wp_send_json_error('Geçersiz rol. Sadece Editör ve Katılımcı rolleri atanabilir.');
    }
...
public function ajax_get_users() {
    ...
    $user_data = array();
    foreach ($users as $user) {
        // Kullanıcı meta bilgilerini al
        $first_name = get_user_meta($user->ID, 'first_name', true);
        $last_name = get_user_meta($user->ID, 'last_name', true);
        
        // Rol isimlerini Türkçe olarak çevir
        $role_names = array();
...
public function ajax_get_performances() {
    ...
    wp_send_json_success($performances);
}
...
public function ajax_save_company_settings() {
    ...
    wp_send_json_success('Firma ayarları başarıyla kaydedildi.');
}
...
public function ajax_upload_company_logo() {
    ...
    if ($movefile && !isset($movefile['error'])) {
        ...
        wp_send_json_success(array('logo_url' => $movefile['url']));
    } else {
        wp_send_json_error('Logo yüklenemedi: ' . $movefile['error']);
    }
}

public function ajax_remove_company_logo() {
    delete_option('bkm_company_logo');
    wp_send_json_success('Logo başarıyla kaldırıldı.');
}
